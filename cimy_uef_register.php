...
function cimy_change_login_registration_logo() {
	...
	if (!empty($options["registration-logo"])) {
		...
		$bg_url = $cuef_upload_webpath.basename($options["registration-logo"]);
		list($logo_width, $logo_height, $logo_type, $logo_attr) = getimagesize($bg_url);
		?>
		<style type="text/css">
		#login h1:first-child a:first-child {
			background: url(<?php echo esc_url($bg_url); ?>) no-repeat top center;
